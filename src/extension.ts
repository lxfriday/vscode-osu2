...
function init(config: vscode.WorkspaceConfiguration, activeTheme: ThemeConfig) {
  ...
  ;(cursorExploder = new CursorExploder(activeTheme)),
    (comboMeter = new ComboMeter(config))

  plugins.push(cursorExploder, comboMeter)

  plugins.forEach((plugin) => plugin.onDidChangeConfiguration(config))

  documentChangeListenerDisposer = vscode.workspace.onDidChangeTextDocument(
...
function onDidChangeTextDocument(event: vscode.TextDocumentChangeEvent) {
  ...
  plugins.forEach((plugin) =>
    plugin.onDidChangeTextDocument(combo, osumode, event)
  )
}
